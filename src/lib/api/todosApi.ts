...
class TodosApiService {
  static async getAll() {
    const response = await client.get(`todos`).catch((error) => {
      throw new Error(error.message)
    })
    return response.data
  }
  static async post(todo: PostTodoItem) {
    await client.post(`todos`, todo).catch((error) => {
      throw new Error(error.message)
    })
  }
  ...
  static async patch(todo: TodoItem) {
    await client.patch(`todos/${todo.id}`, todo).catch((error) => {
      throw new Error(error.message)
    })
  }
  static async delete(todo: TodoItem) {
    await client.delete(`todos/${todo.id}`, { data: todo }).catch((error) => {
      throw new Error(error.message)
    })
  }
}
